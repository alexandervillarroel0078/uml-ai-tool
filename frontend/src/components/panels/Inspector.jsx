...
export default function Inspector({
  ...
  //   onDetailsChange?.({ attrs: [created, ...(details?.attrs || [])] });
  // }
  // async function patchAttr(aid, patch) {
  //   const updated = await updateAttribute(aid, patch);
  //   const next = (details?.attrs || []).map((x) => (x.id === aid ? updated : x));
  ...
  //   onDetailsChange?.({ meths: [created, ...(details?.meths || [])] });
  // }
  // async function patchMeth(mid, patch) {
  //   const updated = await updateMethod(mid, patch);
  //   const next = (details?.meths || []).map((x) => (x.id === mid ? updated : x));
  //   onDetailsChange?.({ meths: next });
  // }
  // async function removeMeth(mid) {
  //   await deleteMethod(mid);
  //   const next = (details?.meths || []).filter((x) => x.id !== mid);
  //   onDetailsChange?.({ meths: next });
  // }
  ...
  //           <div
  //             key={a.id}
  //             style={{ display: "grid", gridTemplateColumns: "1.2fr 1fr 84px 40px", gap: 8, alignItems: "center" }}
  //           >
  //             <input
  ...
  //     {/* Cabecera */}
  //     <div style={{ display: "grid", gridTemplateColumns: "1.2fr 1fr 40px", gap: 8, marginTop: 6, fontSize: 12, opacity: 0.7 }}>
  //       <div>Nombre</div>
  //       <div>Retorno</div>
  //       <div> </div>
  //     </div>
