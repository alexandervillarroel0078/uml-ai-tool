...
//     setMsg("");
//     try {
//       const { data } = await api.post("/diagrams", { title: title.trim() });
//       setTitle("");
//       // recarga y navega directo al dashboard del nuevo
//       await load();
//       nav(`/diagram/${data.id}`);
//     } catch (e) {
...
//           <label style={{ fontSize: 12, opacity: .8 }}>Título</label>
//           <input
//             value={title}
//             onChange={(e) => setTitle(e.target.value)}
//             placeholder="Mi primer diagrama"
//             style={{
//               width: "100%", height: 40, padding: "0 12px", borderRadius: 8, border: "1px solid #334",
//               background: "#0e1526", color: "#fff", boxSizing: "border-box"
//             }}
//           />
//           <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
//             <button type="submit" style={{ padding: "8px 12px", borderRadius: 8, border: "none", background: "#4f46e5", color: "#fff" }}>
//               Crear
//             </button>
...
//           <div>Cargando…</div>
//         ) : diagrams.length === 0 ? (
//           <div style={{ opacity: .7, fontSize: 14 }}>No hay diagramas.</div>
//         ) : (
...
//                     <div style={{ fontSize: 12, opacity: .7 }}>{d.id}</div>
//                   </div>
//                   <div style={{ display: "flex", gap: 8 }}>
//                     <button
//                       type="button"
//                       onClick={() => nav(`/diagram/${d.id}`)}   // abrir dashboard
...
//             style={{ padding: "6px 10px", borderRadius: 8, border: "1px solid #334", background: "transparent", color: "inherit", opacity: page <= 1 ? .5 : 1 }}
//           >
//             ←
//           </button>
//           <span style={{ fontSize: 12, opacity: .8 }}>Página {page} / {totalPages}</span>
//           <button
//             disabled={page >= totalPages}
//             onClick={() => setPage(p => p + 1)}
//             style={{ padding: "6px 10px", borderRadius: 8, border: "1px solid #334", background: "transparent", color: "inherit", opacity: page >= totalPages ? .5 : 1 }}
//           >
//             →
//           </button>
//         </div>
...
// 👇 usa tus helpers del módulo de diagrams
import {
...
export default function HomePage() {
  ...
  async function load() {
    ...
    setMsg("");
    try {
      const data = await apiListDiagrams({ page, limit }); // { items, total, ... }
      setDiagrams(data?.items ?? []);
      setTotal(data?.total ?? 0);
    } catch (e) {
      setMsg(e?.response?.data?.detail || "Error al cargar diagramas");
  ...
  async function createDiagram(e) {
    ...
    try {
      ...
      nav(`/diagram/${d.id}`);     // navega al nuevo
    } catch (e) {
      setMsg(e?.response?.data?.detail || "No se pudo crear");
    }
  }
